...
import numpy as np
from Projects.Functions.Data_cleaning.Data_cleaning import dataset_categorical_values,dataset_numerical_values


def category_data_analysis(dataframe: pd.DataFrame, target: str):
    cats = dataset_categorical_values(dataframe)
    print("cats")
    print(cats)
...
def distribution_analysis_for_int_and_float_features(dataframe: pd.DataFrame, save_path="vide"):
    liste_float_int=dataset_numerical_values(dataframe)
...
def checking_normality_of_features(dataframe: pd.DataFrame, alpha:float, test):
    ...
    for col in liste_float_int:
        ...
        texte=f" feature {col} : \n stats:{stats} \n p_value:{p_value} \n > Distribution normale des données (Test de {str(test)}) : Mise en place des hypothèses (alpha) = {(alpha)}:" +"\n" \
              + "H0 > (les données suivent une distribution normale)" +"\n"+ "H1 > (les données ne suivent pas une distribution normale) \n" \
              +"\n" + f"Ci-dessous les champs du dataset qui ne semblent pas suivre une distribution suivant le test de {str(test)}"
        infos=liste
